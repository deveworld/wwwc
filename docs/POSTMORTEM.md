...
이것은 research_plan.md §9.2에서 예측한 failure mode에 해당한다.
...
즉, **"frozen LM + LoRA write"로는 long-context memory가 되지 않는다.** 작동하는 write 메커니즘들은 더 깊은 아키텍처 변경을 필요로 하며, 그 경우 "frozen LM 위의 allocation law"라는 논문의 전제가 무너진다.
...
2. **Write-decode distribution mismatch는 치명적이다.** 이론적 경고(§9.2)와 부합한다.
...
- **"왜 LoRA write가 long-context recall에서 실패하는가"**: negative result paper 소재
