...
이 둘은 "당연히 그렇겠지"로 읽히는 예비 검증일 뿐, 논문의 headline이 될 수 없다.
...
논문 서사는 H1/H2를 빠르게 확인한 뒤, **모든 지면을 H3/H4/H5에 집중**해야 한다.
...
- ICLR author guide: appendix는 리뷰어가 읽을 의무 없음.
...
리뷰 2의 포지셔닝: "Full-context는 너무 느리고, RAG는 multi-hop에서 무너지지만, Hybrid는 현실적 예산 구간에서 최적 방어선(Pareto frontier)을 형성한다." Hybrid가 무조건 최고일 필요는 없다. 특정 budget 구간에서의 Pareto 우위를 보이면 된다.

### 약점 3: Theory 약함 (전원 지적, 중간)

toy theory(§4.2)의 expected error 분해 = 기초 볼록최적화. "trivially obvious"라는 공격 예상.
...
E4B(5B) 단독은 generalization 의심 가능.

**최소 방어:** RULER 핵심 1-2개를 26B A4B로 돌려 appendix에 "Scale-invariant allocation law" 한 문단.

**완화 요인:** qTTT(Qwen3-4B), PERK(0.5B/7B), In-Place TTT(4B) 등 ICLR 2026 accepted에서도 5B급 실험은 fatal하지 않음.
...
**핵심:** "first hybrid memory" (위험) → "controlled cross-store allocation law under unified budget" (안전). 기존 연구들이 substrate 내 allocation에 머물렀다면, TriStore는 substrate 간 allocation을 다룬다는 구분.

---

## 5. Rebuttal 예상 질문

실제 리뷰에서 거의 반드시 나올 confound 질문들 (리뷰 4, Codex 공통 지적):
...
H5를 방어하려면 **동일 budget에서 fixed split (0/100, 50/50, 100/0) vs learned allocation** 비교가 필수.
...
5. **Core ablation 세트:**
   ...
   - Budget grid별 동일 경향 유지 → 특정 budget에서만 이기는 게 아님을 증명
...
9. **Interior split figure** — budget 변화에 따라 optimal write/cache 비율이 이동하는 phase diagram. 논문의 심장.
...
이 figure 없이는 논문이 성립하지 않는다. X축 = budget (또는 write/cache split ratio), Y축 = accuracy. Mixed slice에서 0% write도 100% write도 아닌 중간 지점이 최적이라는 것이 핵심 evidence.
